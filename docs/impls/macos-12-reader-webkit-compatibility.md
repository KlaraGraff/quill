...
> 状态：**v2.0.2 真机反馈已补齐 Safari 15.1 运行时缺口；v2.0.3 Monterey packaged `.app` 复验待执行**
>
> 定稿日期：2026-07-19
>
...
- EPUB 始终使用本地 `groupBy` helper；ZIP 解压优先系统 `DecompressionStream`，缺失时使用
  已安装的 `fflate.Inflate` 流式 adapter；
...
v2.0.2 在真实 Safari 15.1 上继续停留于 `Preparing book...` 后，能力模拟确认还存在第二组运行时缺口：

| 能力 | Safari 首个支持版本 | 影响 |
|---|---:|---|
| `Array.prototype.at` | 15.4 | EPUB 首章资源加载、CFI、PDF.js legacy |
| `Array.prototype.findLast` / `findLastIndex` | 15.4 | 分页器末章与 PDF.js legacy |
| `Object.hasOwn` | 15.4 | `react-markdown` 与 PDF.js legacy |
| `crypto.randomUUID` | 15.4 | Reader 新窗口、AI/查词/学习卡请求 |

这些 API 不会被 Vite/esbuild 的 `safari15` 语法 target 自动降级。Foliate 的章节加载还会吞掉底层异常，
导致外层无法可靠区分“正在准备”和“首章已失败”，因此真机表现可能是持续 Preparing 或空白正文。
...
- `pdfjs-dist` 5.5.207；
- `fflate` 0.8.2（同时用于 MOBI zlib 与旧 WebKit EPUB ZIP fallback）。
...
v2.0.2 使用 `zip-core-native` 内嵌的 zlib-js Compression Streams 实现；真实 Monterey 反馈后，最终实现
改为 `zip-core-base` 加既有 `fflate.Inflate` adapter。adapter 逐 chunk 输出，仍由 zip.js 按 entry 驱动，
不展开整本 EPUB。
...
- 系统支持 `DecompressionStream('deflate-raw')` 时仍优先原生实现；构造失败或能力缺失才使用 fflate；
- 旧系统按 entry 流式解压，不把整本 EPUB 展开到 JS heap；
- adapter 只实现 EPUB 所需的 `deflate-raw`，继续禁止直接使用 `unzipSync` 全量展开。
...
Foliate 源码内的 `Array.prototype.at` 和 `findLastIndex` 全部改为索引/循环，ZipCrypto 依赖中的单个
`.at(-1)` 在 Rollup 时精确降级；构建门禁阻止 Reader 再次引入这些 API。
...
legacy main/worker 还会直接调用 Safari 15.4 才提供的 `Array.at`、`findLast` 和 `Object.hasOwn`。生成时给
legacy 两件套分别加入同一份最小 runtime prelude；modern 文件不修改，高版本仍走原生实现。
...
| legacy main + worker | 3.34 MB | 697 KB |
| 双构建合计 | 6.32 MB | 1.33 MB |
...
- 阻止 `at`、`findLast`、`findLastIndex`、`Object.hasOwn` 和未保护的 `crypto.randomUUID`；
